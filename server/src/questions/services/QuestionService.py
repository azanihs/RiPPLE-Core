...
def get_course_leaders(user, sort_field, sort_order, limit=25):
    ...
    first_response_SQL = '''SELECT 1 as id, qr.user_id, COUNT(*) as 'total' FROM questions_questionresponse qr,
        ...
        qr.response_id=d.id AND q.author_id=cu.id AND cu.course=%s AND d.isCorrect=1 AND qr.id IN (
            ...
            qr.response_id=d.id AND q.author_id=cu.id AND cu.course=%s GROUP BY qr.user_id,
            q.id)
        GROUP BY qr.user_id'''
