        ...
        <md-card-area md-inset
            ...
            <md-card-content class="fullHeight">
                <div v-if="data.images.length > 0"
                     class="cardPreview fullHeight">
                    <div ref="clamp"
                         class="clamp">
                        <img :src="data.images[0]"></img>{{ data.content }}
                    </div>
                </div>
                <div v-else
                     class="cardPreview fullHeight">
                    <div ref="clamp"
                         class="clamp">{{ data.content }}</div>
                </div>
            </md-card-content>
        </md-card-area>
                ...
                <md-card-content>
                    ...
                    <div class="placeAround">
                        <span class="quality">
                            ...
                            <md-icon :key="star"
                                     v-for="star in getStarIcons(data.quality)">{{ star }}</md-icon>
                        </span>
                        <span>
                            <router-link v-for="topic in data.topics"
                                         :key="topic"
                                         to="/view/questions"
                                         class="topicChipLink">
