<template>
    <md-layout>
        <md-layout>
            <h2>
                <md-icon>school</md-icon> Select topics that you would like to {{ tabName }}
            </h2>
            <md-layout md-flex="100"
                       md-column
                       class="positionRelative">
                <div v-if="+threshold > 0"
                     class="thresholdWrapper"
                     v-bind:style="{marginLeft: threshold + '%'}">
                    <md-tooltip md-direction="top">Required Score To Mentor: {{ threshold }}</md-tooltip>
                </div>
                <md-layout v-for="(topicScore, topicName) in topics"
                           @click.native="selectTopic(topicName)"
                           class="alignMiddle mentorshipWrapper"
                           v-bind:class="{selected: selectedTopics.indexOf(topicName) >= 0, fade: +topicScore < +threshold}"
                           :key="topicName">
        ...
        <md-layout md-flex="100">
            <h2>
                <md-icon>people</md-icon> Review recommendations
            </h2>
            <md-layout md-flex="100">
                <md-layout v-for="peer in peers.recommendations"
                           :key="peer.id">
                    <recommendation-card class="gutter"
                                         :data="peer"
                                         action="Request"></recommendation-card>
                </md-layout>
            </md-layout>
        </md-layout>
        <md-layout md-flex="100">
            <h2>
                <md-icon>person_add</md-icon> Respond to requests
            </h2>
            <md-layout md-flex="100">
                <md-layout v-for="peer in peers.requests"
                           :key="peer.id">
                    <recommendation-card class="gutter"
                                         :data="peer"
                                         action="Accept"></recommendation-card>
                </md-layout>
            </md-layout>
        </md-layout>
    </md-layout>
...
<style>
    /* Global styles to apply to children components */
